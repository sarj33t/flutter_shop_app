...
class CartCubit extends Cubit<CartState> {
  ...
  void addItem(CartItem cartItem) async {
    ...
    final List<CartItem> itemList =
        repository.addItem(cartItem, state.cartItems);
    emit(state.copyWith(status: ApiStatus.success, items: itemList));
  }
  ...
  void removeItem(int productId) {
    emit(state.copyWith(status: ApiStatus.loading));
    final List<CartItem> itemList =
        repository.removeItem(productId, state.cartItems);
    emit(state.copyWith(status: ApiStatus.success, items: itemList));
  }
  ...
  void updateItem(CartItem cartItem) {
    emit(state.copyWith(status: ApiStatus.loading));
    final List<CartItem> itemList =
        repository.updateItem(cartItem, state.cartItems);
    emit(state.copyWith(status: ApiStatus.success, items: itemList));
  }
